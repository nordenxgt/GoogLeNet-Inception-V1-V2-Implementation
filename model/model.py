...
class InceptionModule(nn.Module):
    def __init__(
        ...
        self.branch4 = nn.Sequential(
            ...
            Conv(in_channels, pool_proj, kernel_size=1, stride=1, padding=0)
        )
...
class GoogLeNet(nn.Module):
    def __init__(self, num_classes: int = 1000, aux_flag: bool = True):
        ...
        if aux_flag: self.aux4a, self.aux4d = AuxiliaryClassifier(512, num_classes), AuxiliaryClassifier(528, num_classes)
        else: self.aux4a, self.aux4d = None, None
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self.conv1(x)
        x = F.local_response_norm(self.maxpool1(x), 5)
        
        x = self.conv2(x)
        x = self.conv3(x)
        x = self.maxpool2(F.local_response_norm(x, 5))
